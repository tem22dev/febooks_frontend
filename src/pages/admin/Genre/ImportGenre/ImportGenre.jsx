...
import templateFile from '../../../../assets/uploads/template-genre.xlsx?url';

function ImportGenre(props) {
    const { openImportGenre, setOpenImportGenre } = props;
    ...
    const handleSubmit = async () => {
        setIsLoading(true);
        const res = await bookService.importGenre(dataExcel);

        if (res && res.errCode === 0) {
            message.success(res.message);
            setDataExcel([]);
            setOpenImportGenre(false);
            props.fetchListGenre();
        } else {
            notification.error({
        ...
        >
            ...
            <div style={{ paddingTop: 20 }}>
                <Table
                    ...
                    rowKey="nameGenre"
                    columns={[{ dataIndex: 'nameGenre', title: 'Thể loại' }]}
                />
            </div>
        </Modal>
    );
