...
import templateFile from '../../../../assets/uploads/template-publisher.xlsx?url';

function ImportPublisher(props) {
    const { openImportPublisher, setOpenImportPublisher } = props;
    ...
    const handleSubmit = async () => {
        setIsLoading(true);
        const res = await bookService.importPublisher(dataExcel);

        if (res && res.errCode === 0) {
            message.success(res.message);
            setDataExcel([]);
            setOpenImportPublisher(false);
            props.fetchListPublisher();
        } else {
            notification.error({
        ...
        >
            ...
            <div style={{ paddingTop: 20 }}>
                <Table
                    ...
                    rowKey="namePublisher"
                    columns={[{ dataIndex: 'namePublisher', title: 'Nhà xuất bản' }]}
                />
            </div>
        </Modal>
    );
