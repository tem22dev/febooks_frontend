...
import templateFile from '../../../../assets/uploads/template-supplier.xlsx?url';

function ImportSupplier(props) {
    const { openImportSupplier, setOpenImportSupplier } = props;
    ...
    const handleSubmit = async () => {
        setIsLoading(true);
        const res = await bookService.importSupplier(dataExcel);

        if (res && res.errCode === 0) {
            message.success(res.message);
            setDataExcel([]);
            setOpenImportSupplier(false);
            props.fetchListSupplier();
        } else {
            notification.error({
        ...
        >
            ...
            <div style={{ paddingTop: 20 }}>
                <Table
                    ...
                    rowKey="nameSupplier"
                    columns={[{ dataIndex: 'nameSupplier', title: 'Nhà cung cấp' }]}
                />
            </div>
        </Modal>
    );
