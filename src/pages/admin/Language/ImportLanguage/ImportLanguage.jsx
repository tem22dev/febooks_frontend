...
import templateFile from '../../../../assets/uploads/template-language.xlsx?url';

function ImportLanguage(props) {
    const { openImportLanguage, setOpenImportLanguage } = props;
    ...
    const handleSubmit = async () => {
        setIsLoading(true);
        const res = await bookService.importLanguage(dataExcel);

        if (res && res.errCode === 0) {
            message.success(res.message);
            setDataExcel([]);
            setOpenImportLanguage(false);
            props.fetchListLanguage();
        } else {
            notification.error({
        ...
        >
            ...
            <div style={{ paddingTop: 20 }}>
                <Table
                    ...
                    rowKey="nameLanguage"
                    columns={[{ dataIndex: 'nameLanguage', title: 'Ngôn ngữ' }]}
                />
            </div>
        </Modal>
    );
